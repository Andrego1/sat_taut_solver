...
happyExpList = Happy_Data_Array.listArray (0,43) ([560,70,54,8960,1120,33632,12305,17922,2240,0,220,24576,11265,128,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","Exp","var","\"(\"","\")\"","\"&&\"","\"||\"","\"!\"","\"->\"","\"<->\"","%eof"]
        bit_start = st * 13
        bit_end = (st + 1) * 13
...
happyReduction_7 (HappyTerminal (VAR happy_var_1))
	 =  HappyAbsSyn4
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	VAR happy_dollar_dollar -> cont 5;
	LPAR -> cont 6;
	RPAR -> cont 7;
	AND -> cont 8;
	OR -> cont 9;
